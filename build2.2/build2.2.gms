$title  Nutrition-foodintake Optimization Model
...
$if not setglobal build   $setglobal build 2.2
* evaluate calorie deficit and nutrient intake as rate (not as value):
$if not setglobal rate    $setglobal rate 1
* generate land usage report:
$if not setglobal landrep $setglobal landrep 0
* set scenario by upper limit for cropping area expansion:
...
* set CP demand variation scenario:
$if not setglobal scn_cp  $setglobal  scn_cp 0
* set feed stockpile release scenario:
$if not setglobal scn_fsp $setglobal scn_fsp 0
* set optional import decline (otherwise no import):
$if not setglobal scn_imp $setglobal scn_imp 0

$if not setglobal gdx_results   $setglobal gdx_results   .\results\%build%_results.gdx
$if not setglobal excel_results $setglobal excel_results .\results\%build%_results.xlsx
$if not setglobal rdc_file      $setglobal rdc_file      .\rdc_scn3.gms
...
* classification of crops
    ...
    pasture(c)
    /pbarley,corn,sorghum/
    orchard(c)
    /mandarin,apple,mis_fruits/
    local(c)
    /scane,sbeat,rapeseed/
   
* food groups
    edible(c)
    /rice,wheat,pwheat,barley,naked,mis_grains,sweetp,potato,soy,psoy,mis_beans,green_veges,mis_veges,mandarin,apple,mis_fruits/  
    marine(c)
    /fish,seaweed/
    processed(c)
...
parameter
    ...
    rdc_rate(*)       'Import decline rate    (no import as default)';
    rdc_rate(g) = 1;
...
$if not %scn_imp%==0 $include %rdc_file%


Display data, intake, nvalue, rdc_rate;
...
Equation
    ...
    distbal_stock 'Distribution balance with feed grain stockpile'
    ...
    cbal          'Calorie balance (kca/capita)'
    croc
    dbal          'Net food balance by fgroup (grams/capita)'
    dbalsq     
    droc          'Net food sufficiency rate by fgroup'
    drocsq
    adbal         'Net food balance by fgroup (grams/capita)'
    adbalsq     
    adroc         'Net food sufficiency rate by fgroup'
    adrocsq
    
    obj           'Objective function evaluated as value '
    alt           'Alternative objective function evaluated as rate';
    ...
    dmsupply(fe)..        dms(fe)  =e= fconst(fe,"post")+sum(c, xcrop(c)*x2fe(fe,c));
    ...
    dbalsq(cg)..   deltasq(cg)    =e= sqr(delta(cg));
    ...
    adbalsq(ag)..  adeltasq(ag)   =e= sqr(adelta(ag));
    adroc(ag)..    adeltamn(ag)   =e= (adelta(ag)/sum(ap $amap(ag,ap), nnpc(ap))) / j(ag);
    adrocsq(ag)..  adeltamnsq(ag) =e= sqr(adeltamn(ag));

    obj..   target =e= weight*cdeficit + (1-weight)*(sum(cg,delta(cg))   +sum(ag,adelta(ag)));
    alt..   target =e= weight*crate    + (1-weight)*(sum(cg,deltamnsq(cg)) +sum(ag,adeltamnsq(ag)));

Model crop_constraint   / ybal,fbal,pbal,obal,aybal,afbal,apbal,aobal,albal /;
Model animal_constraint / tdndemand,tdnsupply,tdnbal,cpdemand,cpsupply,cpbal,
                          dmsupply,distbal,bredbal,dairyoxrep,heiferrep,calfrep /;
Model balance           / cbal,croc, dbal,dbalsq,droc,drocsq, adbal,adbalsq,adroc,adrocsq /;

* Standard solution
Model sol_dif     / crop_constraint + animal_constraint + balance + obj /;
Model sol_rat     / crop_constraint + animal_constraint + balance + alt /;
...
* CP demand variation and grain feed stockpile scenario
model sol_cp_low  / sol_dif - cpdemand + cpdemand_low /;
model sol_cp_high / sol_dif - cpdemand + cpdemand_high/;
model sol_stock   / sol_dif - distbal  + distbal_stock/;
...
$sTitle Scenario by weight on calorie balance (w = 100, 75, 50, 25)
* set upper limit for area expansion up to 3X
limit = 3;
...
* set limit = 3, weight = 0.75
    ...
    landrep(w,c) = wxcrop(w,c);
    landrep(w,"total") = sum(c, landrep(w,c));
    landrep("current",c) = data(c,"area");
    landrep("current","total") = sum(c,landrep("current",c));
    


$sTitle Scenario by upper limit for area expansion (w = 2, 3, 4, 5)
* set weight on calorie balance as 0.75
weight = 0.75;
...
$sTitle First-year scenario (cropping area is fixed for the first year)
* set limit = 3, weight = 0.75
limit = 3;
weight = 0.75;
...
$sTitle Stockpile release scenario (Estimated maximum duration of stockpile release)
...
$sTitle CP demand variation scenario
* set limit = 3, weight = 0.75
...
$sTitle Export Report by Scenario
Execute_Unload '%gdx_results%',
wcroprep, wfoodrep, wliverep, wdietrep, radar, aradar,
paddyrep, fieldrep, pasturerep, orchardrep, landrep,
