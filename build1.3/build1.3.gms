...
$if not setglobal build $setglobal build 1.3
...
Equation
   ...
   nutrient(nn)  'Nutrients balance      (kcal grams/capita)'
...
nutrient(nn)..   ndeficit(nn) =e= nreq(nn) - sum(c,xcrop(c)*x2nn(c,nn))/(tpop*(10**6)*365)
...
objective..      target =e= cweight*cdeficit + sum(c,((1-cweight)/7)*delta(c));

* denominator for (1-cweight): count items that take non-zero values or 7 food groups
* scale for objective: value(kcal and gram) or change(percent)
